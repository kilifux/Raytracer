...
Vector getColour(PerspectiveCamera camera, Sphere sphere, Sphere s2, float x, float y, int resX, int resY) {
...
Vector Sampling(PerspectiveCamera camera, Sphere sphere, Sphere s2, int x, int y, int resX, int resY, int maxSteps, float centreX, float centreY, float offset) {
...
void Render(PerspectiveCamera camera, TGABuffer& tgaBuffer, Sphere s1, Sphere s2)
{
    Ray ray = Ray();
    Vector colour = Vector(0, 0, 0);
...
int main(int argv, char** args) {
    ...
    Sphere s1 = Sphere(Vector(0, 0, 0), 3, Vector(0.5f,0.1f,0.3f));
    Sphere s2 = Sphere(Vector(0.5, 0, 0), 2.5, Vector(0.2f,0.7f,0.9f));
    //Triangle t1 = Triangle(Vector(0, 0, 0), Vector(0, 0.5, 0), Vector(0.5, 0, 0));
    //Plane p1;

    TGABuffer tgaBuffer(perspCam.GetResX(), perspCam.GetResY());
    ...
    Render(perspCam, tgaBuffer, s1, s2);
    Render(perspCam, tgaBuffer, s2, s1);
